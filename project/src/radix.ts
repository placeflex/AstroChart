...
class Radix {
  ...
  drawCusps(): void {
    ...
    for (let i = 0, ln = this.data.cusps.length; i < ln; i++) {
      ...
      lines.forEach(function (line) {
        ...
        newLine.setAttribute("stroke-dasharray", "15");
  ...
  aspects(customAspects?: FormedAspect[] | null): Radix {
    ...
    for (let i = 0, ln = aspectsList.length; i < ln; i++) {
      ...
      if (!duplicateCheck.includes(opositeKey)) {
        ...
        const startCircle = createCircle(x1, y1, 3, this.settings.LINE_COLOR);
        const endCircle = createCircle(x2, y2, 3, this.settings.LINE_COLOR);
        ...
        if (this.settings.ADD_LINE_CIRCLES) {
          wrapper.appendChild(startCircle);
          wrapper.appendChild(endCircle);
        }
      }
    }
  ...
  drawCircles(): void {
    ...
    if (!this.settings.DISABLED_INNER_CIRCLE) {
      wrapper.appendChild(circle);
    }
